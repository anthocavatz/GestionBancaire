package fr.moff.gestionbanquaire.tools;

import android.database.sqlite.SQLiteDatabase;
import android.database.sqlite.SQLiteOpenHelper;
import android.util.Log;

import fr.moff.gestionbanquaire.Ado.ClientADO;
import fr.moff.gestionbanquaire.Ado.CompteADO;
import fr.moff.gestionbanquaire.Ado.OperationADO;
import fr.moff.gestionbanquaire.classes.Client;
import fr.moff.gestionbanquaire.classes.Compte;
import fr.moff.gestionbanquaire.classes.Operation;

import static fr.moff.gestionbanquaire.activities.LoginActivity.getContext;

/**
 * Created by Julian on 16/01/2018.
 */

public class DBHelper extends SQLiteOpenHelper {
    //version number to upgrade database version
    //each time if you Add, Edit table, you need to change the
    //version number.
    private static final int DATABASE_VERSION =1;
    // Database Name
    private static final String DATABASE_NAME = "gestionBancaire.db";
    private static final String TAG = DBHelper.class.getSimpleName().toString();

    public DBHelper( ) {
        super(getContext(), DATABASE_NAME, null, DATABASE_VERSION);
    }

    @Override
    public void onCreate(SQLiteDatabase db) {
        //All necessary tables you like to create will create here
        db.execSQL(ClientADO.createTable());
        db.execSQL(CompteADO.createTable());
        db.execSQL(OperationADO.createTable());
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        Log.d(TAG, String.format("SQLiteDatabase.onUpgrade(%d -> %d)", oldVersion, newVersion));

        // Drop table if existed, all data will be gone!!!
        db.execSQL("DROP TABLE IF EXISTS " + Client.TABLE);
        db.execSQL("DROP TABLE IF EXISTS " + Compte.TABLE);
        db.execSQL("DROP TABLE IF EXISTS " + Operation.TABLE);
        onCreate(db);
    }

}
